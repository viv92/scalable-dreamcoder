...
from utils_scdr_slicedLatents_taskEmbFrozen_noFantasy_seddU import *
...
if __name__ == '__main__':
    ...
    program_latent_seqlen = 32 # 128 # 64
    task_latent_seqlen = 32 # 128
    ...
    num_sampling_steps = program_latent_seqlen # int(program_latent_seqlen * 0.25)
    ...
    hyperparam_dict = {}
    hyperparam_dict['method'] = 'scdr_taskFSQ_fantAppend_FsqDitThresh_seddU' 
    ...
    while train_step < num_train_steps + train_steps_done:
        ...
        if (sleep_mode == 0) and (len(test_dataset) > 0): # wake mode - get solved test data 
            ...
            with torch.no_grad():
                ...
                x_sample, flip_count = get_sample(dit, program_latent_seqlen, dit_vocab_size, num_sampling_steps, task_embs.shape[0], task_embs, cfg_scale, 0, device) # x_sample.shape: [b, seqlen]
                ...
                with torch.no_grad():
                    ...
                    while len(new_fantasy_dataset) < min_fantasies:
                        ...
                        with torch.no_grad():
                            ...
                            # get sample tokens corresponding to indices of codebook 
                            x_sample, flip_count = get_sample(dit, program_latent_seqlen, dit_vocab_size, num_sampling_steps, task_embs.shape[0], task_embs, cfg_scale, sampling_eps, device) # x_sample.shape: [b, seqlen]
                            x_sample = x_sample.flatten() # shape: [b * seqlen]
...
The format of input and output has to be inferred from the examples as well.'''
                        ...
                        results_flip_count.append(flip_count)
...
The format of input and output has to be inferred from the examples as well.'''
            ...
            #     new_train_dataset += test_dataset[:100]
        ...
        if sleep_mode == 2: # dream mode - train DiT
            ...
            if len(minibatch) > 0:
                ...
                if dream_on_what != -1:
                    ...
                    # perturb the data
                    x_perturb = perturb(x, sigma, dit_vocab_size)
                    ...
                    # calculate loss 
                    dit_loss = score_entropy_loss(log_score, sigma.unsqueeze(-1), x_perturb, x, dit_vocab_size)
        ...
        if sleep_mode_counter == 1:
            ...
            if sleep_mode == 1: # sleep mode - plot for FSQ
                ...
                # plot flip counts
                if len(results_flip_count) > 0:
                    fig = plt.figure()
                    plt.plot(results_flip_count, label='sampling_flips')
                    plt.legend()
                    plt.title('val:{}'.format(results_flip_count[-1]))
                    save_path = results_dir + 'plot_trainStep=' + str(train_step) + '_sleepMode=' + str(0) + '_flips.png'
                    fig.savefig(save_path)
                    plt.close(fig)
            ...
            if sleep_mode == 2: # 2: # dream mode - plot for DIT 
                ...
                save_ckpt(device, task_fsq_ckpt_path, task_fsq, dream_optimizer)
