...
def prepare_arc_data_square_prompts(chal_file, sol_file, n):
    dataset = []
    ...
    for qid in eval_chal_dict.keys():
        ...
        for i in range(len(eval_chal_dict[qid]['train'])):
            ...
            if x_shape[0] == n and x_shape[1] == n and y_shape[0] == n and y_shape[1] == n:
                task_x.append(arc_list_to_string(x))
                task_y.append(arc_list_to_string(y))

        # create test dataset items
        task_x_test, task_y_test = [], []
        for i in range(len(eval_chal_dict[qid]['test'])):
            ...
            y_shape = np.array(y).shape 
            if x_shape[0] == n and x_shape[1] == n and y_shape[0] == n and y_shape[1] == n:
                task_x_test.append(arc_list_to_string(x))
                task_y_test.append(arc_list_to_string(y))

        L = len(task_x) * len(task_x_test) 
        if L > 0: # valid square task

            item = {}
            item['inputs'] = task_x + task_x_test 
            item['outputs'] = task_y + task_y_test 
...
'''
            for i in range(len(item['inputs']) - 1): # -1 to avoid test input-output pair in the prompt
                inp = item['inputs'][i]
                out = item['outputs'][i]
                prompt += 'Input\n\n' + inp + '\n\n' + 'Output\n\n' + out + '\n\n'  
...
if __name__ == '__main__':
    ...
    program_latent_seqlen = 8 # 32 # 128 # 64
    task_latent_seqlen = 8 # 32 # 128
    ...
    dropout = 0.1 
    weight_decay = 0.1 
    compress_factor = 10 # 0 # 1 # 0.01
    reconstruction_factor = 1
    wake_factor = 0 # 1 
    task_compress_factor = 1
    task_reconstruction_factor = 0.1
    task_loss_factor = 0.1 # 0.5
    ...
    min_fantasies = 100
    all_edit_fantasy_buffer_size = 10 ** 2
    train_buffer_size = 10 ** 3
    delay_start_iter = -1 # sum(sleep_steps_list)
    fantasy_gen_cycle_counter = -1
    fantasy_gen_skip_cycles = 1 

    sampling_eps = 0.1 # 0.3 # 0
    sleep_loss_threshold = 0.1 # 0.05 # 0.02 # 0.2 
    dream_loss_threshold = 2.5 # 5.5 # 8
    timeout = 60 * 60 
    fant_timeout = 60 * 15 # 5
    init_train_dataset_size = 100 # 10 # 1000
    ...
    hyperparam_dict = {}
    hyperparam_dict['method'] = 'scdr_taskFSQ_fantAppend_FsqDitThresh_infuse100' 
    hyperparam_dict['fsqD'] = fsq_d_model
    # hyperparam_dict['fsqL'] = fsq_n_layers
    hyperparam_dict['pLatSeq'] = program_latent_seqlen
    hyperparam_dict['TfsqD'] = te_d_model
    hyperparam_dict['TLatSeq'] = task_latent_seqlen
    ...
    hyperparam_dict['TCF'] = task_compress_factor
    hyperparam_dict['TRF'] = task_reconstruction_factor
    hyperparam_dict['TLF'] = task_loss_factor
    ...
    hyperparam_dict['Fmin'] = min_fantasies 
    ...
    square_dim = 3
    results_dir = './results/verify_arc_sq' + str(square_dim) + '_' + hyperparam_str + '/'
    ckpts_dir = './ckpts/'
    dit_ckpt_path = ckpts_dir + 'dit_' + hyperparam_str + '.pt'
    fsq_ckpt_path = ckpts_dir + 'fsq_' + hyperparam_str + '.pt'
    task_fsq_ckpt_path = ckpts_dir + 'task_fsq_' + hyperparam_str + '.pt'
    ...
    # prepare arc dataset 
    train_dataset = []
    arc_folder = '/home/vivswan/experiments/arc_needs_system2/arc-prize-2024/'
    arc_train_chal_file = arc_folder + 'arc-agi_training_challenges.json'
    arc_train_sol_file = arc_folder + 'arc-agi_training_solutions.json'
    test_dataset = prepare_arc_data_square_prompts(arc_train_chal_file, arc_train_sol_file, square_dim)
    ...
    # init task FSQ 
    task_fsq_encoder = init_task_fsq_encoder_transformer(t5_d_model, t5_max_seq_len, te_d_model, te_d_k, te_d_v, te_n_heads, te_n_layers, te_d_ff, dropout, latent_dim, device)
    task_fsq_decoder = init_task_fsq_decoder_transformer(latent_dim, t5_max_seq_len, te_d_model, te_d_k, te_d_v, te_n_heads, te_n_layers, te_d_ff, dropout, t5_d_model, device)
    task_fsq = Task_FSQ_Transformer(device, num_quantized_values, task_fsq_encoder, task_fsq_decoder, t5_max_seq_len, task_latent_seqlen).to(device)

    # init dit 
    dit_x_seq_len = program_latent_seqlen
    dit_max_seq_len = dit_x_seq_len + 1 # [t, x]
    dit_condition_dim = latent_dim # t5_d_model
    ...
    dream_params = list(dit.parameters()) + list(task_fsq.parameters())
    ...
    task_fsq, _ = load_ckpt(task_fsq_ckpt_path, task_fsq, dream_optimizer, device=device, mode='train')
    ...
    results_task_loss, results_task_recon_loss, results_task_compress_loss, results_task_codebook_unique, results_task_codebook_usage, results_task_masked_latents = [], [], [], [], [], []
    ...
    while train_step < num_train_steps + train_steps_done:
        ...
        if not (num_switches == 0):
            ...
            if (sleep_mode == 1):
                if time() - start_time > timeout:
                    ...
                    if (sleep_mode_counter > sleep_steps_list[sleep_mode]) and ( (len(results_train_loss) == 0) or (results_train_loss[-1] < sleep_loss_threshold) ): 
                        switch_required = 1
                        start_time = time()
            ...
            if (sleep_mode == 2):
                if time() - start_time > timeout:
                    ...
                    if (sleep_mode_counter > sleep_steps_list[sleep_mode]) and ( (len(results_dit_loss) == 0) or (results_dit_loss[-1] < dream_loss_threshold) ): 
                        switch_required = 1
        ...
        if (sleep_mode == 0) and (len(test_dataset) > 0): # wake mode - get solved test data 
            ...
            with torch.no_grad():
                ...
                cap_tokens_dict = t5_tokenizer(cap_batch, return_tensors='pt', padding='max_length', truncation=True)
                ...
                # get task embeddings
                task_embs, _, _, _, _, _ = task_fsq.quantize(task_fsq.encode(cap_embs)) # [b * t5_max_seq_len, latent_dim]
                task_embs = task_embs.unflatten(dim=0, sizes=(-1, t5_max_seq_len)) # [b, t5_max_seq_len, latent_dim]
                task_embs = task_embs[:, :task_latent_seqlen]

                # get sample tokens corresponding to indices of codebook 
                x_sample = get_sample(dit, program_latent_seqlen, dit_mask_token, dit_vocab_size, num_sampling_steps, task_embs.shape[0], task_embs, cfg_scale, 0, device) # x_sample.shape: [b, seqlen]
                ...
                with torch.no_grad():
                    ...
                    while len(new_fantasy_dataset) < min_fantasies:
                        ...
                        with torch.no_grad():
                            ...
                            # get sample tokens corresponding to indices of codebook 
                            x_sample = get_sample(dit, program_latent_seqlen, dit_mask_token, dit_vocab_size, num_sampling_steps, task_embs.shape[0], task_embs, cfg_scale, sampling_eps, device) # x_sample.shape: [b, seqlen]
                            x_sample = x_sample.flatten() # shape: [b * seqlen]
...
The format of input and output has to be inferred from the examples as well.'''
            ...
            if len(train_dataset) > 0:
                while len(edit_fantasy_dataset) < min_fantasies:
    ...
    The format of input and output has to be inferred from the examples as well.'''
        ...
        if sleep_mode == 2: # dream mode - train DiT
            ...
            if len(minibatch) > 0:
                ...
                if dream_on_what != -1:
                    ...
                    # get task embeddings - with gradients on
                    task_embs, task_z_e, task_z_q, task_usage, task_unique, task_idx = task_fsq.quantize(task_fsq.encode(cap_embs)) # [b * t5_max_seq_len, latent_dim]
                    task_embs = task_embs.unflatten(dim=0, sizes=(-1, t5_max_seq_len)) # [b, t5_max_seq_len, latent_dim]

                    ## loss for task FSQ 

                    recon_cap_embs, task_percent_masked_latents = task_fsq.decode(task_embs) 
                    task_recon_loss = task_reconstruction_factor * task_reconstruction_loss(recon_cap_embs, cap_embs)
                    task_compress_loss = task_compress_factor * compression_loss(task_z_e)
                    task_loss = task_recon_loss + task_compress_loss 
                    ...
                    condition = task_embs[:, :task_latent_seqlen]
                    ...
                    ## total dream loss 
                    dream_loss = dit_loss + task_loss_factor * task_loss 
                    ...
                    # bookeep losses
                    results_dit_loss = ema(results_dit_loss, dream_loss.item()) # NOTE that thresholding will be based on dream loss, not dit loss
                    results_task_loss.append(task_loss.item())
                    results_task_codebook_usage.append(task_usage.item())
                    results_task_codebook_unique.append(task_unique)
                    results_task_masked_latents.append(task_percent_masked_latents.item())
                    results_task_recon_loss.append(task_recon_loss.item())
                    results_task_compress_loss.append(task_compress_loss.item())
        ...
        if sleep_mode_counter == 1:
            ...
            if sleep_mode == 2: # 2: # dream mode - plot for DIT 
                ...
                # plot task lossses 
                if len(results_task_loss) > 0:

                    fig, ax = plt.subplots(2,2, figsize=(15,10))

                    ax[0,0].plot(results_task_recon_loss, label='task_recon_loss')
                    ax[0,0].plot(results_task_compress_loss, label='task_compress_loss')
                    ax[0,0].plot(results_task_loss, label='task_loss')
                    ax[0,0].legend()
                    ax[0,0].set(xlabel='eval_iters')
                    ax[0,0].set_title('task:{:.3f} recon:{:.3f} compress:{:.3f}'.format(results_task_loss[-1], results_task_recon_loss[-1], results_task_compress_loss[-1]))
                    ax[0,0].set_ylim([0, 10])

                    ax[1,0].plot(results_task_codebook_unique, label='task_codebook_unique')
                    ax[1,0].legend()
                    ax[1,0].set(xlabel='eval_iters')
                    ax[1,0].set_title('val:{:.3f}'.format(results_task_codebook_unique[-1]))

                    ax[0,1].plot(results_task_codebook_usage, label='task_codebook_usage')
                    ax[0,1].legend()
                    ax[0,1].set(xlabel='train_iters')
                    ax[0,1].set_title('val:{:.3f}'.format(results_task_codebook_usage[-1]))

                    ax[1,1].plot(results_task_masked_latents, label='percent_task_masked_latents')
                    ax[1,1].legend()
                    ax[1,1].set(xlabel='train_iters')
                    ax[1,1].set_title('val:{:.3f}'.format(results_task_masked_latents[-1]))

                    # plt.suptitle('final_train_loss: ' + str(results_train_loss[-1]))
                    save_path = results_dir + 'plot_trainStep=' + str(train_step) + '_sleepMode=' + str(1) + '_taskFSQ.png'
                    plt.savefig(save_path)
                    plt.close(fig)
